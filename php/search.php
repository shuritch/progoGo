...
<section id = "sec" style="min-height:1000px; height: auto; display: flex; justify-content: flex-start; align-items: center; flex-direction: column;">
	...
	for ($i=0; $i < $Num; $i++) { 
		if(strpos(mb_strtolower($app_name[$i]), mb_strtolower($search)) !== false or strpos(mb_strtolower($app_razel[$i]), mb_strtolower($search)) !== false or strpos(mb_strtolower($app_about[$i]), mb_strtolower($search)) !== false or strpos(mb_strtolower($app_downloader[$i]), mb_strtolower($search)) !== false){
		echo '
		<div class="app">

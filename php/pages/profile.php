...
<head>
	...
	<title>profile | progogo.com</title>
	<meta charset="utf-8">
...
<body>
	...
	if (isset($_COOKIE['ClientInfo'])) {
		...
		$sql = 'SELECT email,admin, password FROM users';
...
<div id="webchat">
	...
	<div id="writing">
		...
		if(isset($_COOKIE['ClientInfo']) and $access == 'true'){
			...
			echo 'style="background: url('.$thisrows[0].') no-repeat center center; background-size:contain; "';}?> id="sign"><?php if(isset($_COOKIE['ClientInfo'])){echo '
...
<div id="websiteversion">V0.1 <span>born</span></div>
<div class="empty"></div>
...
<section id = "sec">
	<h2>Тут блок с прогами скачиваемость которых высокая или новые</h2>
</section>
<div id="profileblock">
	
</div>
<section style="opacity: 0;" id = "lastSection">
...
signInButton.on('click', () => {
	...
	else if(isset($_COOKIE['ClientInfo']) and $access == 'true'){
		$link = mysqli_connect("localhost", "root", "", "progogo");
	$sql = 'SELECT downloadcount FROM apps';
	$result = mysqli_query($link, $sql);
	$rows = mysqli_fetch_all($result, MYSQLI_ASSOC);
	...
	foreach ($rows as $row) {
		...
		$link = mysqli_connect("localhost", "root", "", "progogo");
		$sqlcomand = 'SELECT password FROM users WHERE email="'.$userCookieInfo[0].'"';

		$resultcomand = mysqli_query($link, $sqlcomand);
		$thisrows = mysqli_fetch_row($resultcomand);
		mysqli_close($link);
		if($aAccess == 'true'){
			echo '$("#profileblock").append("<div id=profile><div id = statisticcontainer><h2 class=razdel>admin page</h2><div id=circlestat><div class=statisticCircle id=users_count><span>'.$users_count.'</span><p>users</p></div><div class=statisticCircle id=files_count><span>'.$files_count.'</span><p>files</p></div><div class=statisticCircle id=downloads_count><span>'.$app_download_count.'</span><p>downloads</p></div></div></div><div id=controlpanelsection><form enctype= multipart/form-data action = http://progogo/php/admin.php method=post id = controlpanelblock><h2 class= controllrazel>Загрузить приложение</h2><div id=controlform><div class = controlpanel><h2>Название продукта</h2><input type=text name = filename placeholder=название /></div><div class = controlpanel><h2>Описание продукта</h2><input type=text name = about placeholder=название /></div><div class = controlpanel><h2>Картинка Приложения</h2><input id=stuffname type=file name = fileimg style=display:none; /><label for =stuffname >выбрать</label></div><div class = controlpanel><h2>продукт</h2><input id = stuffname2 type=file name = applec style=display:none; /><label for =stuffname2 >выбрать</label></div><div class = controlpanel><h2>раздел</h2><select name=razdel><option disabled>раздел</option><option>security</option><option>system</option><option>systemDesign</option><option>forProgrammist</option><option>other(windows)</option><option>templates</option><option>scripts</option><option>efeects</option><option>imgs</option><option>icons</option><option>fonts</option><option>soft(cybersecurity)</option><option>database</option><option>frontend</option><option>backend</option><option>hacking</option><option>other(learning)</option><option>other</option></select></div><button type=submit class=adminbutton >Создать</button></div></form><form action = http://progogo/php/adminremove.php method = post id = controlpanelblock2><h2 class= controllrazel>Удалить приложение</h2><div id =controlform2><div class = controlpanel><h2>Название продукта</h2><input type=text name = filename placeholder=название /></div><button type=submit class=adminbutton >Удалить</button></div></form></div><a href= http://progogo/ id=adminexit></a></div>");
			...
			$link = mysqli_connect("localhost", "root", "", "progogo");
			$sqlcomand = 'SELECT name FROM users WHERE email="'.$userCookieInfo[0].'"';
			$resultcomand = mysqli_query($link, $sqlcomand);
			$thisrows = mysqli_fetch_row($resultcomand);
			$sqlcomand2 = 'SELECT photo FROM users WHERE email="'.$userCookieInfo[0].'"';
			$resultcomand2 = mysqli_query($link, $sqlcomand2);
			$thisrows2 = mysqli_fetch_row($resultcomand2);
			$sqlcomand3 = 'SELECT downloads FROM users WHERE email="'.$userCookieInfo[0].'"';
			$resultcomand3 = mysqli_query($link, $sqlcomand3);
			$thisrows3 = mysqli_fetch_row($resultcomand3);
			$res = explode('/',$thisrows3[0]);
			$resln = count($res);
			mysqli_close($link);
			echo '$("#profileblock").append("<div id=profile><form enctype= multipart/form-data method = post action = http://progogo/php/redactor.php id = profilecontainer><h2 class = razdel>profile</h2><input type=file name=img id=imginp /><label for=imginp id=profileLogo style= background-image:url('.$thisrows2[0].'); background-size:cover;></label><button id = savechanges type = submit>Сохранить</button><div id=profilefooter><div class=userinfo2><h2>Name:</h2><input  value='.$thisrows[0].' type=text name= name/></div><div class=userinfo2><h2>Password:</h2><input  value='.$pass.' type=text name= password/></div><div class=userinfo2><h2>email:</h2><input value='.$log.' type=text name= email/></div></div></form><a href= http://progogo/ id=adminexit></a><div style = position:relative;><h2 class=razdel>download history</h2></div><div id=downloadblock></div></div>");
			...
			$link = mysqli_connect("localhost", "root", "", "progogo");
			while ($dcount < $resln) {
				$sqlcomand4 = 'SELECT photo FROM apps WHERE name="'.$res[$dcount].'"';
				$resultcomand4 = mysqli_query($link, $sqlcomand4);
				$thisrows4 = mysqli_fetch_row($resultcomand4);
				$sqlcomand4 = 'SELECT app FROM apps WHERE name="'.$res[$dcount].'"';
				$resultcomand4 = mysqli_query($link, $sqlcomand4);
				$thisrows5 = mysqli_fetch_row($resultcomand4);
				echo '$("#downloadblock").append("<div style=background-image:url('.$thisrows4[0].'); class=downloadcon><form action = http://progogo/php/pages/appPage.php method = get><input name =application value = '.$thisrows4[0].' readonly = readonly style = display:none;><button type=submit>'.$res[$dcount].'</button><form></div>");';
				$dcount+=1;
			}
			mysqli_close($link);
		}
	...
	?>
	$('.downloadcon a').on('click', function() {
	setTimeout(function(){
	hr = $(this).attr('href');
	hr = hr.substr(18);
	...
	$.ajax({
		type: "POST",
        url: "http://progogo/php/downloadcount.php",
        data:'action='+ hr,

<?php
	if(isset($_GET['application']) and $_GET['application'] != ''){
		...
		$sqlcomand = 'SELECT name,photo,app,date,razel,about,downloadcount FROM apps WHERE photo ="'.$_GET['application'].'"';
		$resultcomand = mysqli_query($connection, $sqlcomand);
		$thisappinfo = mysqli_fetch_row($resultcomand);
...
<head>
	...
	<?php 
		...
		$sql = 'SELECT name FROM apps';
		$result = mysqli_query($link, $sql);
		$rows = mysqli_fetch_all($result, MYSQLI_ASSOC);
		$application_name = 'progogo,progogo.com,progogo.ru,progogo.net,Лучшие программы,Антивирусы,Ускорить компютер,для программиста,windows soft,безопастность,система,изображения,иконки,шрифты,шаблоны,скрипты,эффекты,внешний вид системы, кибербезопастность, базы данных,progo,pro,programs,free programs,бесплатные программы';
		foreach ($rows as $row) {
   		$application_name = $application_name .','. $row["name"];
		}
		mysqli_close($link);
		echo '<meta name="keywords" content="'.$application_name.'">';
	?>
...
<body>
	...
	for ($i=0; $i < $Num; $i++) { 
		if($visitor_ip == $user_ip[$i]){
			...
			$no_ip = 'yes';
			$today = date("H:i:s");
			$sql2 = "UPDATE online SET visits=visits + 1,date='$today' WHERE ip='$visitor_ip'";
...
<div id = "container">
<section id="main" style="height: 600px;">
	<?php
	echo '
	<div id="HugeImg" style="background: url('.$thisappinfo[1].') no-repeat center center; background-size: contain; height: 80%; width: 80%; position: relative;">
...
<div class="headcap">
	<h2 style="text-align: center;"><?php echo $thisappinfo[0]; ?></h2>
</div>
<section id = "sec" style="height: auto; display: flex; justify-content: flex-start; align-items: center; flex-direction: column;">
	<div id="appCharacter"><div class="character"><?php echo $thisappinfo[3]; ?></div><div class="character"><?php echo $thisappinfo[4]; ?></div><div class="character"><?php echo $thisappinfo[6]; ?> скачиваний</div></div>
	<div id="appabout" style="height: auto; width: calc(100% - 100px); text-align: left; font-size: 0.8em; color: #fff; padding: 50px; display: flex;justify-content: center;align-items: center; margin-top: 50px; background: url(http://progogo/scss/img/classy_fabric.png) repeat center center;" >
		<?php echo $thisappinfo[5];?>

	</div>
	<div id ="downloadsection">
		<div id="downloadsectionHeader"><img src="http://progogo/scss/img/download.png"><h2>Скачать</h2></div>
		<div id="downloadsectionMiddle">
			<div class="pie degree">
 			<span class="block"></span>
 			<span id="time">0</span>
			</div>
			<button id="downloadsectionDownload"><div id="downloadsectionDownloadImg"></div><div id="downloadBut"><h2>Скачать</h2></div></button>
		</div>
		<div id="downloadsectionFooter"><img src="http://progogo/scss/img/equals-sign.png"><h2>Рекомендуем</h2></div>
	</div>
	<div id="randomMaterials">
		 <div class="swiper-container">
    <div class="swiper-wrapper">
    	<?php
    	$conn = mysqli_connect("localhost", "root", "", "progogo");
		$sql = 'SELECT name,about,date,downloadcount,photo,razel,app FROM apps ORDER BY downloadcount DESC';	
    	...
    	?>
      <div class="swiper-slide"><div class="swiperImg" style="background: url(<?php echo $app_photo[0];?>) no-repeat center center;"></div><div class="swiperInfo"><div class="App_Name"><?php echo $app_name[0];?></div><div class="App_About"><?php echo $app_about[0];?></div><form class="App_Download"><input name="application" value="<?php echo $app_photo[0];?>" style="display: none;" /><button type="submit">Скачать</button></form></div></div>
      <div class="swiper-slide"><div class="swiperImg" style="background: url(<?php echo $app_photo[1];?>) no-repeat center center;"></div><div class="swiperInfo"><div class="App_Name"><?php echo $app_name[1];?></div><div class="App_About"><?php echo $app_about[1];?></div><form class="App_Download"><input name="application" value="<?php echo $app_photo[1];?>" style="display: none;" /><button type="submit">Скачать</button></form></div></div>
      <div class="swiper-slide"><div class="swiperImg" style="background: url(<?php echo $app_photo[2];?>) no-repeat center center;"></div><div class="swiperInfo"><div class="App_Name"><?php echo $app_name[2];?></div><div class="App_About"><?php echo $app_about[2];?></div><form class="App_Download"><input name="application" value="<?php echo $app_photo[2];?>" style="display: none;" /><button type="submit">Скачать</button></form></div></div>
    </div>
  </div>
	</div>
</section>
...
$('#downloadsectionDownload').on('click', function() {
	setTimeout(function(){
	hr = '<?php echo $thisappinfo[2]; ?>';
...
signInButton.on('click', () => {
	...
	?>
	$("#downloadsectionDownload").click(function() {
	elem = $('#downloadsectionDownload');
	timerr();
	href = 'http://progogo/app/<?php echo $thisappinfo[2];?>';
	setTimeout("window.location = href; $('#downloadsectionDownload').css('display', 'flex');$('.pie').css('display', 'none');", 15000);
	$("#downloadsectionDownload").css('display', 'none');
  ...
  if(percent<(totaltime/2)){
    deg = 90 + (360*percent/totaltime);
      $('.pie').css('background-image',
                'linear-gradient('+deg+'deg, transparent 50%,  #000 50%),linear-gradient(90deg,  #000 50%, transparent 50%)'
               );
  } else if(percent>=(totaltime/2)){
          deg = -90 + (360*percent/totaltime);
          $('.pie').css('background-image',
                'linear-gradient('+deg+'deg, transparent 50%, #f90 50%),linear-gradient(90deg,  #000 50%, transparent 50%)'
               );
          }
}
var count = 0;
